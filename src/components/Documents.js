...
function Documents() {
			...
			axios
				...
				.then((response) => {
					if (response.status === 200) {
						setDocuments(
							documents.filter((doc) => doc.title !== documentTitle)
						);
					} else {
	...
	const submitEditingUrl = useCallback(
		(event) => {
	...
	const onEditUrlKeydown = useCallback(
		(event) => {
			var code = event.charCode || event.keyCode;
	...
	const renderAddByUrl = useMemo(() => {
		if (isEditingUrl) {
			...
			return (
				<div
					className={cx(
						styles['link-upload-text-container'],
						styles['link-upload-container']
					)}
					onClick={() => editUrl()}
				>
		...
		<div className={styles['general-container']}>
			<div className={cx(styles['side'], styles['left'])}>
				...
				>
					...
					<span className={cx(styles['text'], styles['text-center'])}>
			...
			<div className={cx(styles['side'], styles['right'])}>
				<div className={styles['list-documents-title-container']}>
					<span
						className={cx(styles['list-documents-title-text'], styles['text'])}
					>
						Lista de documentos adicionados
					</span>
				</div>
				<div className={styles['list-documents-container']}>{renderFiles}</div>
			</div>
		</div>
	);
}
